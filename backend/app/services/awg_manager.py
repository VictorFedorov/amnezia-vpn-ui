...
class AWGManager:
    """Менеджер для работы с AmneziaWG (WireGuard) через Docker контейнер"""

    def __init__(self, ssh_manager: SSHManager, container_name: str = "amnezia-awg"):
    ...
    def get_peers(self) -> List[Dict]:
        ...
        """
        try:
            command = f"docker exec {self.container_name} wg show"
    ...
    def _parse_handshake(self, handshake_str: str) -> int:
        ...
        if not handshake_str:
            return 0
        # Попробуем как целое число (unix timestamp)
        try:
            return int(handshake_str)
        except ValueError:
            pass
        # Человекочитаемый формат: "2 minutes, 3 seconds ago"
        total_seconds = 0
        patterns = [
    ...
    def block_peer(self, public_key: str) -> bool:
        ...
        try:
            ...
            # wg set awg0 peer <PUBKEY> remove
            command = f"docker exec {self.container_name} wg set awg0 peer {public_key} remove"
            exit_code, stdout, stderr = self.ssh.execute_command(command)
    ...
    def unblock_peer(self, public_key: str, allowed_ips: str) -> bool:
        ...
        try:
            ...
            # wg set awg0 peer <PUBKEY> allowed-ips <IPS>
            command = f"docker exec {self.container_name} wg set awg0 peer {public_key} allowed-ips {allowed_ips}"
            exit_code, stdout, stderr = self.ssh.execute_command(command)
